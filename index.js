...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\ bv ao grupo *${mdata.subject}* se for menina se apresente com foto, nome e idade! se for menino vsf`
				...
				teks = `saiu e nem mamou os adm🤬 @${num.split('@')[0]} adm esta triste😔`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: 'espera ai to roubando net do vizinho',
				success: 'ai está, desculpe a demora',
				error: {
					stick: 'Eu falhei :( desculpa',
					Iv: 'Desculpe, o link está inválido☹️'
				},
				only: {
					group: 'ESSE COMANDO E PRA GRUPO CARAI KKkkk.',
					ownerG: 'PILANTRA DEIXA O ARABE SABER DISSO.kkkk',
					ownerB: 'PILANTRA DEIXA O ARABE SABER DISSO.',
					admin: 'ALA O CARA ACHA Q É ADM KKK',
					Badmin: 'PRECISO SER ADM HEHE🤡',
				}
			}
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️ALA O CARA CURTE LOFI KKKKK'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('safadao kkk')
					reply(mess.wait)
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('55996673834@s.whatsapp.net', options, text, {quoted: mek})
                ...
                case 'logo3d':
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'carai é poema ?kkk', text, {quoted: mek})
			...
			case 'mapa': 
				...
				case 'audio':
					...
					if (args.length < 2) return client.sendMessage(from, 'A PRONTO AGORA TENHO QUE ADIVINHAR OQ VC QUER Q EU FALE'?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('MEU PAU QUE VOU FALAR TUDO ISSO KKK')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('mlk burro, tenta dnv depois kkk ')
				...
				case 'clearall':
				case 'limpar':
					if (!isOwner) return reply('pronto lindu')
				...
				case 'adicionar':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'abrir') {
					    reply(`mais ja abriu? nem descansei`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(`pronto agora deixa eu descansar`)
            ...
            case 'criador':
            case 'keke':
                  client.sendMessage(from, {displayname: "Arabe adm", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Este é o número do meu criador ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
			...
			case 'remover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('remover quem tio kk')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
				...
				case 'toimg':
				case 'converter':
					if (!isQuotedSticker) return reply('cade a figurinha? kkkk burrão ')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('algo de erro ;(, tente dnovo dps')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'pronto lindu' })
						fs.unlinkSync(ran)
					})
					break
				case 'nsfw':
				...
				case 'boasvindas':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('welcome ativado com sucesso')
						welkom.push(from)
