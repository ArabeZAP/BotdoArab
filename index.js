...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\ bv ao grupo *${mdata.subject}* se apresenta e mama o arabe`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬🤬❭ESPERA UM POUCO IRMAO❬🤬❭',
				success: '️❬ ✔ ❭ TA AI MANO',
				error: {
					stick: 'Eu falhei :( desculpa',
					Iv: 'Desculpe, o link está inválido☹️'
				},
				only: {
					group: '❬🥴❭ ESSE COMANDO E PRA GRUPO CARAI KK.❬🥴❭ ',
					ownerG: '❬🤬❭ PILANTRA DEIXA O ARABE SABER DISSO.❬🤬❭ ',
					ownerB: '❬🤬❭  PILANTRA DEIXA O ARABE SABER DISSO.❬🤬❭  ',
					admin: ' ❬ 🥴 ❭ O CARA ACHA Q E ADM KKK ❬ 🥴 ❭ ',
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️bota um modao ai po'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
			...
			case 'mapa': 
				...
				case 'clearall':
				case 'limpar':
					if (!isOwner) return reply(' af kkk')
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'boasvindas':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('af agora vou falar bv')
						welkom.push(from)
